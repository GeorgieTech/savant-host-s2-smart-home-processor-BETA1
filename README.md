# CRYPT — S2 Smart Home Processor V1.1.10
...
- **Settings** page: LAN discovery of other CRYPT chassis. Each host is unique by Savant UID, so a third box appears on its own. **Live** is heard on the wire; **Linked** merges libraries both ways. **Unison TOSLINK** (optional) plays the same track on every linked optical jack, steered by Time Clock
