...
- **Library** page: upload with progress, browse by artist / album / track, playlists, and Manage for bulk delete (with progress) plus artist/album/genre edits. A viewer host can list another CRYPT host’s catalog (`/data/crypt/peers.json`); files stay on the shelf and Play copies a track over first
