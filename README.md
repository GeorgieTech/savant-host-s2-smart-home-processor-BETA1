# CRYPT — S2 Smart Home Processor V1.1.12
...
- **Settings** page: LAN discovery of other CRYPT chassis. Each host is unique by Savant UID, so a third box appears on its own. **Live** is heard on the wire; **Linked** merges libraries both ways. **Unison TOSLINK** (optional) plays the same track on every linked optical jack. The follower rides CRYPT/1 CLOCK instead of seeking on the ~400 ms path delay (that seek was the two-zone echo). Discovery and clock use multicast `239.18.20.1:41880` with JSON broadcast as fallback. Library merge skips a full JSON refetch when the catalog fingerprint is unchanged.
