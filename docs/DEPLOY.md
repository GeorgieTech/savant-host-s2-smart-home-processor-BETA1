...
```sh
scp -O host-webui/index.html host-webui/library.html host-webui/eq.html host-webui/crypt.css \
  host-webui/server.py host-webui/player.py host-webui/library.py host-webui/wave.py \
...
cp /tmp/index.html /tmp/library.html /tmp/eq.html /tmp/crypt.css \
  /tmp/server.py /tmp/player.py /tmp/library.py /tmp/wave.py /tmp/pin-hostname.sh \
...
- DualLite / 1 GB — keep V1.1.3 to library + TOSLINK. No extra daemons. No SSC expanders.
