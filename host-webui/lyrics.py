...
CLIENT = "CRYPT/1.0 (https://github.com/GeorgieTech/savant-host-s2-smart-home-processor-BETA1)"
...
def _payload(source, parsed, synced, artist="", title="", album=""):
...
def _cache_key(rel, artist, title, duration):
    raw = "%s|%s|%s|%s" % (rel or "", artist or "", title or "", int(duration or 0))
...
class LyricsIndex(object):
    ...
    def lookup(self, rel, fetch=False, duration=0):
        rel = (rel or "").replace("\\", "/").lstrip("/")
        full = _full_audio(rel)
        if not full:
            return {"ok": False, "error": "not found", "lines": []}
        ...
        for path in sidecar_paths(full):
            ...
            if path.lower().endswith(".lrc") or _TS.search(text):
                ...
                if parsed["lines"]:
                    return _payload("sidecar", parsed, True, artist, title, album)
            parsed = parse_plain(text)
            if parsed["lines"]:
                return _payload("sidecar", parsed, False, artist, title, album)
        embedded = _embedded_lyrics(full)
        if embedded.strip():
            if _TS.search(embedded):
                parsed = parse_lrc(embedded)
                if parsed["lines"]:
                    return _payload("tags", parsed, True, artist, title, album)
            parsed = parse_plain(embedded)
            if parsed["lines"]:
                return _payload("tags", parsed, False, artist, title, album)
        if fetch:
    ...
    def _fetch_lrclib(self, rel, full, artist, title, album, duration):
        ...
        key = _cache_key(rel, artist, title, duration)
        cache_path = os.path.join(LYRICS_DIR, key + ".json")
        try:
        ...
        if synced.strip() and _TS.search(synced):
            ...
            payload = _payload("lrclib", parsed, True, artist, title, album)
        elif plain.strip():
            parsed = parse_plain(plain)
            payload = _payload("lrclib", parsed, False, artist, title, album)
        else:
            return None
        try:
            tmp = cache_path + ".tmp"
