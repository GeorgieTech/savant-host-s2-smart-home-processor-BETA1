...
class FollowPlanTests(unittest.TestCase):
    ...
    def test_ahead_below_catch_does_not_seek(self):
    ...
    def test_ahead_catch_still_rare(self):
        plan, err = player.follow_plan(10.2, 10.0, last_seek_age=9.0, good_age=9.0)
        self.assertEqual(plan, "seek")
        self.assertAlmostEqual(err, -0.2, places=4)
        plan, err = player.follow_plan(10.2, 10.0, last_seek_age=9.0, good_age=1.0)
        self.assertEqual(plan, "slew")

    def test_slew_pause_is_short_fraction(self):
        """Soft SIGSTOP: fraction of the lead, cap well below the old 80 ms."""
        ...
        self.assertLess(player.slew_ahead_seconds(-0.05), 0.05)
...
class FollowHeardSlewTests(unittest.TestCase):
    ...
    def test_slight_ahead_does_not_start_slew(self):
        ...
        self.assertEqual(p._sync_slew_at, 0.0)
    ...
    def test_slew_ahead_caps_old_eighty(self):
        p = self._bare()
        p.paused = False
        p.proc = type("Proc", (object,), {"pid": 1, "poll": lambda self: None})()
        sleeps = []
        kills = []
        ...
        orig_killpg = player.os.killpg
        orig_getpgid = player.os.getpgid
        player.time.sleep = fake_sleep
        player.os.killpg = fake_killpg
        player.os.getpgid = fake_getpgid
        try:
            self.assertTrue(p._slew_ahead(0.08, gen=1))
        finally:
            player.time.sleep = orig_sleep
            player.os.killpg = orig_killpg
            player.os.getpgid = orig_getpgid
        self.assertEqual(sleeps, [player.SLEW_MAX_S])
        self.assertEqual(len(kills), 2)
        self.assertAlmostEqual(p.t0, player.SLEW_MAX_S, places=4)
